...
export function ProveedorView({ proveedor }: ProveedorViewProps) {
  return (
    <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap items-center gap-3 justify-center sm:justify-start">
            <span className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1 text-[10px] font-black uppercase tracking-widest ${
              ...
              <div className={`h-1.5 w-1.5 rounded-full ${proveedor.activo ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500'}`} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <InfoCard icon={<Hash size={20} />} label="NIT" value={proveedor.nit || 'No registrado'} />
        <InfoCard icon={<User size={20} />} label="Nombre del Contacto" value={proveedor.nombreContacto} />
        <InfoCard icon={<Phone size={20} />} label="Teléfono" value={proveedor.telefono} />
        <InfoCard icon={<Mail size={20} />} label="Correo Electrónico" value={proveedor.correo || 'No registrado'} />

        <div className="md:col-span-2">
          <InfoCard icon={<MapPin size={20} />} label="Dirección" value={proveedor.direccion || 'Sin dirección registrada'} />
        </div>
        ...
        {proveedor.createdAt && (
          <InfoCard
            ...
            value={new Date(proveedor.createdAt).toLocaleDateString()}
          />
        )}
      </div>
    </div>
