...
import { ConfirmModal } from './components/ConfirmModal.view'
...
interface ProveedoresPageViewProps {
  ...
  confirmModal: { isOpen: boolean; type: 'activar' | 'desactivar'; proveedor: Proveedor | null }
  onCloseConfirm: () => void
  onConfirm: () => void
  onCreate: () => void
...
export function ProveedoresPageView({
  ...
  return (
    <div className="flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-2 duration-700">
      {feedbackMessage && !isFormModalOpen && !isViewModalOpen && !confirmModal.isOpen && (
      ...
      <ConfirmModal
        ...
        nombre={confirmModal.proveedor?.empresa ?? ''}
      />
    </div>
  )
}
